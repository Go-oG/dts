...
class MinBoundingCircle {
  final Geometry _input;

  late List<Coordinate> _extremalPts;

  Coordinate? _centre;

  double _radius = 0.0;

  MinBoundingCircle(this._input);
  ...
  Geometry getMaximumDiameter() {
    ...
    switch (_extremalPts.length) {
      ...
      case 2:
        return _input.factory.createLineString2([_extremalPts[0], _extremalPts[1]]);
      default:
        return _input.factory.createLineString2(_farthestPoints(_extremalPts));
    }
  }
  ...
  static List<Coordinate> _farthestPoints(List<Coordinate> pts) {
  ...
  Geometry getDiameter() {
    ...
    return _input.factory.createLineString2([p0, p1]);
  }
  ...
  void _computeCirclePoints() {
    ...
    if (_input.getNumPoints() == 1) {
      List<Coordinate> pts = _input.getCoordinates();
      _extremalPts = [Coordinate.of(pts.first)];
      return;
    }
    Geometry convexHull = _input.convexHull();
    List<Coordinate> hullPts = convexHull.getCoordinates();
    List<Coordinate> pts = hullPts;
    if (hullPts[0].equals2D(hullPts[hullPts.length - 1])) {
      pts = hullPts.sublist(0, hullPts.length - 1).map((e) => e.copy()).toList();
    }
    if (pts.length <= 2) {
    ...
    for (int i = 0; i < pts.length; i++) {
      ...
      if (Angle.isObtuse(P, R, Q)) {
        _extremalPts = [Coordinate.of(P), Coordinate.of(Q)];
        return;
        ...
        _extremalPts = [Coordinate.of(P), Coordinate.of(Q), Coordinate.of(R)];
        return;
      }
    }
  ...
  static Coordinate _lowestPoint(List<Coordinate> pts) {
  ...
  static Coordinate? _pointWitMinAngleWithX(List<Coordinate> pts, Coordinate P) {
  ...
  static Coordinate? _pointWithMinAngleWithSegment(List<Coordinate> pts, Coordinate P, Coordinate Q) {
