...
class RelateNG {
  static bool relate4(Geometry a, Geometry b, TopologyPredicate pred,
    ...
    return RelateNG(a, false, bnRule).evaluate3(b, pred);
  }

  static bool relate3(Geometry a, Geometry b, String imPattern) =>
      RelateNG(a, false).evaluate2(b, imPattern);

  static IntersectionMatrix relate(Geometry a, Geometry b,
          [BoundaryNodeRule? bnRule]) =>
      RelateNG(a, false, bnRule).evaluate(b);

  static RelateNG prepare(Geometry a, [BoundaryNodeRule? bnRule]) =>
      RelateNG(a, true, bnRule);
  ...
  bool evaluate3(Geometry b, TopologyPredicate predicate) {
    ...
    computeAtPoints(geomB, RelateGeometry.kGeomB, _geomA, topoComputer);
    if (topoComputer.isResultKnown()) {
      return topoComputer.getResult();
    }
    computeAtPoints(_geomA, RelateGeometry.kGeomA, geomB, topoComputer);
    if (topoComputer.isResultKnown()) {
      return topoComputer.getResult();
    }
  ...
  bool hasRequiredEnvelopeInteraction(Geometry b, TopologyPredicate predicate) {
    ...
    if (predicate.requireCovers(RelateGeometry.kGeomA)) {
      if (!_geomA.getEnvelope().covers(envB)) {
        return false;
      }
      isInteracts = true;
    } else if (predicate.requireCovers(RelateGeometry.kGeomB)) {
  ...
  void computePP(RelateGeometry geomB, TopologyComputer topoComputer) {
    ...
    for (Coordinate ptB in ptsB) {
      if (ptsA.contains(ptB)) {
        ...
        topoComputer.addPointOnPointExterior(RelateGeometry.kGeomB, ptB);
      }
      if (topoComputer.isResultKnown()) {
        return;
      }
    ...
    if (numBinA < ptsA.length) {
      topoComputer.addPointOnPointExterior(RelateGeometry.kGeomA, null);
    }
  }

  void computeAtPoints(RelateGeometry geom, bool isA, RelateGeometry geomTarget,
  ...
  bool computePoints(RelateGeometry geom, bool isA, RelateGeometry geomTarget,
      TopologyComputer topoComputer) {
  ...
  void computePoint(bool isA, Coordinate pt, RelateGeometry geomTarget,
      TopologyComputer topoComputer) {
    int locDimTarget = geomTarget.locateWithDim(pt);
    int locTarget = DimensionLocation.location(locDimTarget);
    int dimTarget = DimensionLocation.dimension2(
        locDimTarget, topoComputer.getDimension(!isA));
    topoComputer.addPointOnGeometry(isA, locTarget, dimTarget, pt);
  }

  bool computeLineEnds(RelateGeometry geom, bool isA, RelateGeometry geomTarget,
      TopologyComputer topoComputer) {
    ...
    while (geomi.moveNext()) {
      ...
      if (elem is LineString) {
        ...
        hasExteriorIntersection |=
            computeLineEnd(geom, isA, e0, geomTarget, topoComputer);
        if (topoComputer.isResultKnown()) {
          return true;
        }
        if (!line.isClosed()) {
          ...
          hasExteriorIntersection |=
              computeLineEnd(geom, isA, e1, geomTarget, topoComputer);
          if (topoComputer.isResultKnown()) {
            return true;
          }
        }
  ...
  bool computeLineEnd(
    ...
    int dimLineEnd = DimensionLocation.dimension2(
        locDimLineEnd, topoComputer.getDimension(isA));
    ...
    int dimTarget = DimensionLocation.dimension2(
        locDimTarget, topoComputer.getDimension(!isA));
    topoComputer.addLineEndOnGeometry(
        isA, locLineEnd, locTarget, dimTarget, pt);
    return locTarget == Location.exterior;
  }

  bool computeAreaVertex(RelateGeometry geom, bool isA,
    ...
    while (geomi.moveNext()) {
      ...
      if (elem is Polygon) {
        ...
        Polygon poly = elem;
        hasExteriorIntersection |= computeAreaVertex2(
            geom, isA, poly.getExteriorRing(), geomTarget, topoComputer);
        if (topoComputer.isResultKnown()) {
          return true;
        }
        for (int j = 0; j < poly.getNumInteriorRing(); j++) {
          hasExteriorIntersection |= computeAreaVertex2(
              geom, isA, poly.getInteriorRingN(j), geomTarget, topoComputer);
          if (topoComputer.isResultKnown()) {
            return true;
          }
        }
  ...
  void computeAtEdges(RelateGeometry geomB, TopologyComputer topoComputer) {
    ...
    List<RelateSegmentString> edgesB =
        geomB.extractSegmentStrings(RelateGeometry.kGeomB, envInt);
  ...
  void computeEdgesAll(List<RelateSegmentString> edgesB, Envelope envInt,
      EdgeSegmentIntersector intersector) {
    List<RelateSegmentString> edgesA =
        _geomA.extractSegmentStrings(RelateGeometry.kGeomA, envInt);
    final edgeInt = OpEdgeSetIntersector(edgesA, edgesB, envInt);
    edgeInt.process(intersector);
  }

  void computeEdgesMutual(List<RelateSegmentString> edgesB, Envelope envInt,
      EdgeSegmentIntersector intersector) {
    if (_edgeMutualInt == null) {
      Envelope? envExtract = (_geomA.isPrepared()) ? null : envInt;
      final edgesA =
          _geomA.extractSegmentStrings(RelateGeometry.kGeomA, envExtract);
      _edgeMutualInt = MCIndexSegmentSetMutualIntersector(edgesA, envExtract);
    }
