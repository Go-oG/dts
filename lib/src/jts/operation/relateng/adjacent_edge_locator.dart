...
class AdjacentEdgeLocator {
  List<List<Coordinate>>? _ringList;
  ...
  int locate(Coordinate p) {
    ...
    for (List<Coordinate> ring in _ringList!) {
  ...
  void addSections(Coordinate p, List<Coordinate> ring, NodeSections sections) {
  ...
  NodeSection createSection(Coordinate p, Coordinate prev, Coordinate next) {
    if ((prev.distance(p) == 0) || (next.distance(p) == 0)) {
      debugPrint("Found zero-length section segment");
    }
    return NodeSection(true, Dimension.A, 1, 0, null, false, prev, p, next);
  }

  void addRings(Geometry geom, List<List<Coordinate>> ringList2) {
    if (geom is Polygon) {
      Polygon poly = geom;
  ...
  void addRing(LinearRing ring, bool requireCW) {
    final pts = RelateGeometry.orient(ring.getCoordinates(), requireCW);
    _ringList!.add(pts);
  }
}
