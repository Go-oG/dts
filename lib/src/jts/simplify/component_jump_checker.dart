...
class ComponentJumpChecker {
  ...
  bool hasJump(TaggedLineString line, int start, int end, LineSegment seg) {
    ...
    for (TaggedLineString comp in _components) {
      ...
      if (sectionEnv.intersectsCoordinate(compPt)) {
  ...
  bool hasJump2(TaggedLineString line, LineSegment seg1, LineSegment seg2,
      LineSegment seg) {
  ...
  static bool hasJumpAtComponent2(Coordinate compPt, TaggedLineString line,
      int start, int end, LineSegment seg) {
  ...
  static int crossingCount2(
      Coordinate compPt, LineSegment seg1, LineSegment seg2) {
    RayCrossingCounter rcc = RayCrossingCounter(compPt);
  ...
  static int crossingCount3(
      Coordinate compPt, TaggedLineString line, int start, int end) {
    RayCrossingCounter rcc = RayCrossingCounter(compPt);
  ...
  static Envelope computeEnvelope(LineSegment seg1, LineSegment seg2) {
    ...
    env.expandToIncludeCoordinate(seg1.p0);
    env.expandToIncludeCoordinate(seg1.p1);
    env.expandToIncludeCoordinate(seg2.p0);
    env.expandToIncludeCoordinate(seg2.p1);
    return env;
  }
  ...
  static Envelope computeEnvelope2(TaggedLineString line, int start, int end) {
    Envelope env = Envelope();
    for (int i = start; i <= end; i++) {
      env.expandToIncludeCoordinate(line.getCoordinate(i));
    }
    return env;
  }
}
