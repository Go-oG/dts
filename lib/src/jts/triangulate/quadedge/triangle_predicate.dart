...
class TrianglePredicate {
  static bool isInCircleNonRobust(
    ...
    bool isInCircle = ((((((a.x * a.x) + (a.y * a.y)) * triArea(b, c, p)) -
  ...
  static bool isInCircleDDFast(
      Coordinate a, Coordinate b, Coordinate c, Coordinate p) {
    DD aTerm =
        DD.sqrS(a.x).selfAdd(DD.sqrS(a.y)).selfMultiply(triAreaDDFast(b, c, p));
    DD bTerm =
        DD.sqrS(b.x).selfAdd(DD.sqrS(b.y)).selfMultiply(triAreaDDFast(a, c, p));
    DD cTerm =
        DD.sqrS(c.x).selfAdd(DD.sqrS(c.y)).selfMultiply(triAreaDDFast(a, b, p));
    DD pTerm =
        DD.sqrS(p.x).selfAdd(DD.sqrS(p.y)).selfMultiply(triAreaDDFast(a, b, c));
  ...
  static DD triAreaDDFast(Coordinate a, Coordinate b, Coordinate c) {
    DD t1 = DD
        .valueOf(b.x)
        .selfSubtract2(a.x)
        .selfMultiply(DD.valueOf(c.y).selfSubtract2(a.y));
    DD t2 = DD
        .valueOf(b.y)
        .selfSubtract2(a.y)
        .selfMultiply(DD.valueOf(c.x).selfSubtract2(a.x));
